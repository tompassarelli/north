# Shader interfaces and data flow

## Decisions and rationale

Decide language version and coordinate spaces before writing the data interface. Attributes vary per vertex, varyings interpolate, uniforms are shared values, and textures carry sampled data. Re-normalize interpolated normals. Custom output must participate in the consumer's tone-mapping/color pipeline when required.
...
const material = new THREE.RawShaderMaterial({
  // Declare recognized matrix uniforms in GLSL; renderer supplies their values.
  vertexShader: `
...
const material = new THREE.ShaderMaterial({
  ...
  // WebGL2 makes several former WebGL1 extensions core.
  // Inspect installed material/renderer support instead of copying legacy flags.
