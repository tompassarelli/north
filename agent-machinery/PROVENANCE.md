...
contain `north:THIRD_PARTY_NOTICES.md`; its Caveman section does not pertain to
this package, while the Three.js attribution is retained below. Copying the
full file is therefore not required. The applicable full license texts are retained as
...
## Included no-license Three.js source material

The `threejs-*` skills were imported from
`https://github.com/CloudAI-X/threejs-skills` at commit
`b1c623076c661fc9b03dac19292e825a5d106823`. The upstream README labels the
collection “MIT License”, but the tracked tree at that revision contains no
standalone license or copyright notice. The material is retained here under
the local MIT default recorded by North's source notice; this is not a claim
about an upstream copyright holder's terms. The package carries this source,
revision, and scope attribution in both this file and `agent-machinery:NOTICE`.
