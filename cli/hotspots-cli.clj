...
(defn coordinator-reads [since]
  (->> (sh "journalctl" "-u" "north-coord.service" "--since" since "--no-pager")
       str/split-lines
...
(defn render-coordinator [rows]
  (println (bold "COORDINATOR READS") (dim "— source: journalctl north-coord.service, slow-read attribution"))
  (if (empty? rows)
