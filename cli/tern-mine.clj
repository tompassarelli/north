;; tern-mine.clj <port> [--dry-run] [--full] [--root DIR] [--report FILE] [--limit N] [--verbose]
;;
...
;; state (path -> mtime/size) lives in ~/.local/state/tern/tern-mine/state.edn so the
...
(defn emit-facts! [port {:keys [stem session-id cwd last-ts error-count
  ...
  (let [te (str "@mine:" stem)
    ...
    (tern.coord/put! port te "kind" "mine")
    (tern.coord/put! port te "session_id" (or session-id stem))
    (when cwd (tern.coord/put! port te "repo" (tilde cwd)))
    (when last-ts (tern.coord/put! port te "at" last-ts))
    (when (pos? error-count) (tern.coord/put! port te "error_count" (str error-count)))
    (doseq [v (take MAX-VOTES-PER-UNIT (keys votes))]
      (tern.coord/append! port te "verb_vote" v))
...
;; ---------------------------------------------------------------------------
;; incremental state
(def state-file (io/file home ".local/state/tern/tern-mine/state.edn"))
...
(defn report-md [units meta]
  (let [agg (fn [k] (apply merge-with + (map k units)))          ; name -> total hits
    ...
    (str "# Telemetry baseline — " (:date meta) "\n\n"
         "Mined by `tern-mine` over " (:files meta) " transcript files ("
...
;; ---------------------------------------------------------------------------
(defn -main [& args]
  (let [port (Integer/parseInt (or (first args) tern.coord/PORT))
        opts (set (rest args))
        ...
        todo (->> all
                  ...
                  (#(if limit (take limit %) %)))
        _ (binding [*out* *err*]
            (println (format "tern-mine: %d transcript files, %d to scan%s"
