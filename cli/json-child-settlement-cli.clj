...
(def indexed-query-in-domain (ns-resolve 'north.coord 'indexed-query-in-domain))
(def show-envelope (ns-resolve 'north.coord 'show-envelope))

;; The coordinator's per-query row ceiling. Overflow REFUSES rather than
...
(defn- malformed! [detail]
  (throw (ex-info (str "coordinator returned a malformed " detail)
...
(defn- row-limit? [error]
  (= :indexed-query-row-limit (:type (ex-data error))))
...
(defn- both-domains [port query]
  (vec (distinct (concat (:ok (indexed-query-in-domain port :coordination query max-rows))
                         (:ok (indexed-query-in-domain port :telemetry query max-rows))))))
...
;; coordinator wide enough to overflow that join still gets an exact answer,
;; one indexed per-subject read at a time, instead of a refusal.
(defn- child-facts [port coordinator]
  (let [joined (try
                 (rows-of-width (:ok (indexed-query port (child-fact-query coordinator) max-rows))
                                3 "child fact row")
    ...
    (if joined
      ...
      (let [subjects (filterv child-subject?
                              (mapv first
                                    (rows-of-width
                                     (:ok (indexed-query port (child-subject-query coordinator) max-rows))
                                     1 "child subject row")))]
