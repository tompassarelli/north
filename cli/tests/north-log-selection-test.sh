...
# "the caller did not pin BEAGLE_STORE_LOG" AND "BEAGLE_STORE_TELEMETRY_LOG is unset". The
# packaged wrapper exports BEAGLE_STORE_TELEMETRY_LOG, so the branch never ran, BEAGLE_STORE_LOG
...
resolved() { # resolved <env assignments...> -> "BEAGLE_STORE_LOG|BEAGLE_STORE_TELEMETRY_LOG"
  env -u BEAGLE_STORE_LOG -u BEAGLE_STORE_TELEMETRY_LOG "$@" bash -c '
...
got="$(resolved "HOME=$fixture" "BEAGLE_STORE_TELEMETRY_LOG=$state/telemetry.log")"
check "preset BEAGLE_STORE_TELEMETRY_LOG still selects coordination.log" \
...
# 3. An explicitly pinned BEAGLE_STORE_LOG is still honoured — the caller wins.
got="$(resolved "HOME=$fixture" "BEAGLE_STORE_LOG=$state/facts.log")"
check "explicit BEAGLE_STORE_LOG is never overridden" \
  "$state/facts.log|" "$got"

# 4. An explicit telemetry route is preserved, not clobbered by the derivation.
got="$(resolved "HOME=$fixture" "BEAGLE_STORE_TELEMETRY_LOG=$state/custom-telemetry.log")"
check "explicit BEAGLE_STORE_TELEMETRY_LOG survives coordination selection" \
