#!/usr/bin/env bb
;; Rung 0 caller surface — the steer gate must not conflate an unreadable
...
(let [port (free-unbound-port)
      ;; A `steer`-subject send drives require-live-steer! before any write.
      result (proc/shell
              {:continue true :out :string :err :string
               :extra-env {"FRAM_LOG" "/tmp/north-steer-gate-test.log"
              ...
              "bb" msg-cli (str port) "send" "director" "some-target" "steer" "body")]
  (check! "unreadable steer projection exits 3 (read-unavailable), not 2 (negative)"
          (= 3 (:exit result)))
...
(let [failed (remove second @checks)]
  ...
  (if (seq failed)
    (do (println (str "steer gate read-unavailable: " (count failed) " FAILED"))
        (System/exit 1))
    (println (str "steer gate read-unavailable: PASS (" (count @checks) " checks)"))))
