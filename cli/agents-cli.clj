...
(defn cmd-spawn-help []
  (let [roles (sort (keys (or (orchestration-routing) {})))]
    ...
    (println)
    (println "Usage:")
    (println "  north spawn <role> \"<prompt>\" [routing options] [--composition JSON|@file] [--dry-run]")
    (println)
...
(defn cmd-spawn [args]
  ...
  (let [{:keys [dry? notify provider target model taskGrade domains topology tier reasoning posture composition
    ...
    (cond
      ...
      (do (println (red (str "bespoke composition " (:id selected-composition) " requires --rationale or composition.bespokeReason"))) (System/exit 1))
      (and bespoke? (nil? contract-value))
      (do (println (red (str "bespoke composition " (:id selected-composition) " requires --contract JSON|@file or composition.contract"))) (System/exit 1))
