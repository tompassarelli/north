...
(def VOCAB
  [;; --- executable schema meta-predicates (the schema describes itself) ---
   ...
   ["graph_text_experiment_version" "single" "literal" "version of the graph/text session assignment contract"]
   ["graph_text_experiment_status" "single" "literal" "graph/text experiment eligibility and assignment status"]
   ["graph_text_experiment_arm" "single" "literal" "effective graph, text, or none session arm"]
   ["graph_text_experiment_applied" "single" "literal" "whether the experiment changed the executable session affordance"]
   ["graph_text_experiment_reason" "single" "literal" "typed reason for assignment, pin preservation, or exclusion"]
   ["graph_text_experiment_assignment_sha256" "single" "literal" "SHA-256 committing the graph/text arm assignment"]
   ...
   ["run_arm" "single" "literal" "legacy graph/text authoring arm; compatibility input only, superseded by learning_axis and learning_arm_id"]
   ...
   ["codex_turn_units" "single" "literal" "Codex-only turn count (thread 019f9c36): one Codex turn per input frame, with all tool calls nested inside it. NOT comparable to num_turns — never join the two across providers"]
   ["codex_tool_items" "single" "literal" "count of completed non-agent_message Codex items (tool/command/file-change/MCP calls) inside codex_turn_units — the honest activity signal a turn count cannot provide"]
   ["codex_turn_metric_comparable" "single" "literal" "always \"false\": explicit disclaimer that codex_turn_units/codex_tool_items are not the same measurement as num_turns"]
