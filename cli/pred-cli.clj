...
(def VOCAB
  [;; --- executable schema meta-predicates (the schema describes itself) ---
   ...
   ["watchdog_reason" "single" "literal" "typed cause for a North-initiated watchdog abort"]
   ...
   ["composition_kind" "single" "literal" "managed Orchestration composition kind (template|bespoke); absent on native sessions"]
   ["composition_id" "single" "literal" "Orchestration template or bespoke composition id"]
   ["composition_overrides" "single" "literal" "JSON array of template axes deliberately overridden"]
   ["composition_override_reason" "single" "literal" "full rationale for a template override"]
   ["nearest_template" "single" "literal" "nearest catalog template considered for a bespoke composition"]
   ["bespoke_reason" "single" "literal" "why no catalog template honestly fits a bespoke composition"]
   ["promotion_candidate" "single" "literal" "whether a bespoke composition should be considered for promotion"]
   ["composition_contract_sha256" "single" "literal" "SHA-256 fingerprint of a bespoke Orchestration authority contract"]
   ...
   ["applied_capability" "multi" "literal" "canonical Orchestration capability actually enforced for a run"]
   ["applied_comms_contract_sha256" "single" "literal" "SHA-256 fingerprint of the Orchestration communications contract applied to a run"]
   ["context_tokens" "single" "literal" "current context token use for an agent or run"]
   ...
   ["outcome"        "single" "literal" "terminal result for a thread or run"]
   ["process_outcome" "single" "literal" "terminal state of the provider process or preflight"]
   ["delivery_outcome" "single" "literal" "delivery state: unverified, run-scoped reported, or blocked"]
   ...
   ;; --- ordinary-operation learning assignment + construction receipts ---
   ["learning_assignment_version" "single" "literal" "version of the deterministic pre-provider learning assignment contract"]
   ["learning_policy_version" "single" "literal" "version of the learning policy schema used for this episode"]
   ["learning_policy_sha256" "single" "literal" "SHA-256 of the exact learning policy used for this episode"]
   ["learning_mode" "single" "literal" "ordinary-operation regime: frozen or learning"]
   ["learning_evidence_mode" "single" "literal" "learning observation posture: discovery or evaluation"]
   ["learning_experiment_id" "single" "literal" "stable experiment namespace for an ordinary-operation episode"]
   ["learning_episode_id" "single" "literal" "stable episode identity used by deterministic assignment"]
   ["learning_task_signature_sha256" "single" "literal" "privacy-bounded SHA-256 of the task identity used for cohorting"]
   ["learning_task_signature_coverage" "single" "literal" "coverage of the task identity receipt: exact, partial, or unknown"]
   ["learning_risk" "single" "literal" "run-local P0-P3 risk class, or unknown, at assignment time"]
   ["learning_arm" "single" "literal" "assignment class: control or explore"]
   ["learning_axis" "single" "literal" "single changed learning axis, or control"]
   ["learning_arm_id" "single" "literal" "provider-neutral arm selected within the learning axis"]
   ["learning_propensity" "single" "literal" "deterministic probability of the exact assigned arm"]
   ["learning_explore_propensity" "single" "literal" "configured probability of exploration after eligibility narrowing"]
   ["learning_narrowing_reason" "single" "literal" "typed reason the episode remained control or selected one exploratory arm"]
   ["learning_baseline_sha256" "single" "literal" "SHA-256 of the exact provider-neutral control baseline"]
   ["learning_options_sha256" "single" "literal" "SHA-256 of the exact eligible arm set after risk and floor narrowing"]
   ["learning_assignment_sha256" "single" "literal" "SHA-256 committing the complete immutable learning assignment"]
   ["prompt_receipt_version" "single" "literal" "version of the content-addressed prompt construction receipt"]
   ["prompt_receipt_sha256" "single" "literal" "SHA-256 committing prompt modules, branch traces, parameters, and wire bytes"]
   ["prompt_wire_sha256" "single" "literal" "SHA-256 of the exact UTF-8 prompt bytes sent to the provider"]
   ["prompt_receipt_coverage" "single" "literal" "prompt construction receipt coverage: exact, partial, or unknown"]
   ["environment_receipt_version" "single" "literal" "version of the content-addressed execution-environment receipt"]
   ["environment_receipt_sha256" "single" "literal" "SHA-256 committing tools, hooks, configs, executables, instructions, and skills"]
   ["environment_receipt_coverage" "single" "literal" "execution-environment receipt coverage: exact, partial, or unknown"]
   ["available_skill_catalog_sha256" "single" "literal" "SHA-256 of the skills available to the run, separate from activation"]
   ["activated_resource_closure_sha256" "single" "literal" "SHA-256 of the exact skills and resources activated by the run"]
   ["run_envelope_version" "single" "literal" "version of the run envelope tying assignment, route, prompt, and environment"]
   ["run_envelope_sha256" "single" "literal" "SHA-256 committing assignment, route, adapter/runtime, prompt, and environment receipts"]
   ...
   ["deadline_exceeded_detail" "single" "literal" "bounded structured deadline interrupt detail"]
   ["wire_ledger_version" "single" "literal" "version of the exact append-only WireEvent ledger projection"]
   ["wire_version" "single" "literal" "version of the canonical WireEvent contract"]
   ["wire_ledger_status" "single" "literal" "complete only after every canonical event is durable"]
   ["response_strategy_id" "single" "literal" "response-compression strategy identity, separate from Orchestration routing axes"]
   ...
   ["mcp_operation_receipt" "multi" "literal" "bounded argument-free JSON receipt for one observed MCP operation"]
   ["mcp_operation_aggregate" "multi" "literal" "argument-free JSON count, duration, and failures by MCP operation"]
   ...
   ["native_command_open" "single" "literal" "native commands still open when partial terminal activity was harvested"]
   ["native_command_truncated" "single" "literal" "truncated native commands observed for this run"]
   ["native_command_read" "single" "literal" "native commands classified as text-side reads"]
   ["native_command_edit" "single" "literal" "native commands classified as text-side edits"]
   ...
   ["wire_event_count" "single" "literal" "number of exact events in a complete durable run ledger"]
   ["wire_event_first_sequence" "single" "literal" "first sequence in a complete durable run ledger"]
   ["wire_event_last_sequence" "single" "literal" "last sequence in a complete durable run ledger"]
   ["wire_terminal_event_id" "single" "literal" "event ID of the durable run.terminated boundary"]
   ["wire_ledger_sha256" "single" "literal" "SHA-256 committing the ordered canonical event digests"]
   ["wire_run_lifecycle" "single" "literal" "terminal lifecycle reduced from the exact event sequence"]
   ["wire_termination_code" "single" "literal" "explicit termination code reduced from run.terminated"]
   ["wire_run_id" "single" "literal" "exact branded run ID retained without graph-entity coercion"]
   ["wire_event_id" "single" "literal" "exact branded event ID"]
   ["wire_event_sequence" "single" "literal" "zero-based monotone sequence within one wire run"]
   ["wire_event_at" "single" "literal" "canonical UTC event timestamp"]
   ["wire_event_kind" "single" "literal" "discriminant of the exact WireEvent"]
   ["wire_event_essential" "single" "literal" "whether the event carries required semantics"]
   ["wire_event_json" "single" "literal" "exact canonical JSON bytes of one validated WireEvent"]
   ["wire_event_sha256" "single" "literal" "SHA-256 of the exact canonical WireEvent JSON"]
   ["lifetime_input_tokens" "single" "literal" "cumulative run input tokens"]
   ["lifetime_output_tokens" "single" "literal" "cumulative run output tokens"]
   ["lifetime_cache_read_tokens" "single" "literal" "cumulative run cache-read tokens"]
   ["lifetime_cache_write_tokens" "single" "literal" "cumulative run cache-write tokens"]
   ["lifetime_reasoning_tokens" "single" "literal" "cumulative run reasoning tokens"]
   ["model_call_count" "single" "literal" "structurally observed model calls in the run"]
   ["tool_admitted_count" "single" "literal" "structurally admitted tool calls in the run"]
   ["tool_succeeded_count" "single" "literal" "tool calls terminally succeeded in the run"]
   ["tool_failed_count" "single" "literal" "tool calls terminally failed in the run"]
   ["tool_cancelled_count" "single" "literal" "tool calls terminally cancelled in the run"]
   ["tool_synthetic_failure_count" "single" "literal" "tool calls closed by a synthetic failure terminal"]
   ["run_owner" "single" "literal" "optional owner from the exact run.started event"]
   ["model_tier" "single" "literal" "provider-neutral semantic tier reduced from wire progress"]
   ["usage_terminal_count" "single" "literal" "provider-origin model-call completions observed, including unavailable usage"]
   ["usage_scope" "single" "literal" "Wire replay usage scope; v2 is wire_run_cumulative"]
   ["usage_total_status" "single" "literal" "exact, partial, unknown_incomplete_terminal, or unknown_no_terminal"]
   ...
   ["num_turns"      "single" "literal" "assistant-turn count from complete homogeneous provider terminal witnesses"]
   ["provider_turn_units" "single" "literal" "opaque provider-defined turn-unit count; not comparable to num_turns"]
   ["provider_tool_items" "single" "literal" "provider-defined tool/item activity nested within provider_turn_units when fully witnessed"]
   ["provider_turn_metric_comparable" "single" "literal" "always false: provider_turn_units and provider_tool_items are not assistant-turn metrics"]
   ...
   ["routing_applied_task_grade" "single" "literal" "task grade admitted after routing policy evaluation"]
   ["routing_applied_topology" "single" "literal" "topology admitted after routing policy evaluation"]
   ["routing_applied_tier" "single" "literal" "semantic tier admitted after routing policy evaluation"]
   ["routing_applied_reasoning" "single" "literal" "reasoning level admitted after routing policy evaluation"]
   ["routing_applied_posture" "single" "literal" "posture admitted after routing policy evaluation"]
   ["routing_stock_task_grade" "single" "literal" "stock task grade before a policy-authorized override"]
   ["routing_stock_topology" "single" "literal" "stock topology before a policy-authorized override"]
   ["routing_stock_tier" "single" "literal" "stock semantic tier before a policy-authorized override"]
   ["routing_stock_reasoning" "single" "literal" "stock reasoning level before a policy-authorized override"]
   ["routing_stock_posture" "single" "literal" "stock posture before a policy-authorized override"]
   ["routing_assessment_policy" "single" "literal" "version of the routing assessment policy"]
   ["routing_signal_decision_ownership" "single" "literal" "decision-ownership signal admitted by routing assessment"]
   ["routing_signal_seam_scope" "single" "literal" "seam-scope signal admitted by routing assessment"]
   ["routing_signal_error_exposure" "single" "literal" "error-exposure signal admitted by routing assessment"]
   ["routing_signal_oracle_strength" "single" "literal" "oracle-strength signal admitted by routing assessment"]
   ["routing_signal_foundational_impact" "single" "literal" "foundational-impact signal admitted by routing assessment"]
   ["routing_signal_dependency_shape" "single" "literal" "dependency-shape signal admitted by routing assessment"]
   ["routing_signal_reasoning_shape" "single" "literal" "reasoning-shape signal admitted by routing assessment"]
   ...
   ["authoring_authority_surface" "single" "literal" "run-local managed authoring authority derived from the final executable surface: text, none, or unknown"]
   ...
   ["effective_authority_capability" "multi" "literal" "canonical Orchestration capability compiled into the final provider executable surface"]
   ...
   ["requested_role" "single" "literal" "Orchestration role requested in routing metadata"]
   ...
   ["composition_override" "multi" "literal" "one Orchestration template axis overridden for a run"]
   ["prompt_composition_applied" "single" "literal" "whether a Orchestration prompt composition was applied"]
   ["prompt_capability_class" "single" "literal" "capability class measured for the applied prompt composition"]
   ["applied_role_contract" "single" "literal" "Orchestration role contract actually applied to a run"]
   ["applied_bespoke_contract_sha256" "single" "literal" "SHA-256 of the bespoke Orchestration contract actually applied"]
   ...
   ["applied_template_override" "multi" "literal" "one template axis actually overridden in the applied prompt"]
   ["applied_template_override_reason_sha256" "single" "literal" "SHA-256 of the applied template-override rationale"]
   ...
   ;; --- North Orchestration vocabulary (Orchestration -> North Orchestration migration,
