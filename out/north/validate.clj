...
(defn structural-violations-i [idx ^String subject]
  (let [reference-violations (reduce (fn [violations predicate] (reduce (fn [found target] (if (proj/subject? idx target) found (conj found (str predicate " references missing entity " target)))) violations (proj/string-values-at idx subject predicate))) [] (proj/projectionindex-reference-predicates idx))]
  (reduce (fn [violations predicate] (if (cycle? idx predicate subject) (conj violations (str predicate " cycle")) violations)) reference-violations (proj/projectionindex-acyclic-predicates idx))))
...
(defn work-violations-i [idx ^String te]
  ...
  :else a)) acc (proj/string-values-at idx te p))) [] thread-ref-preds)
   v-ab (reduce (fn [acc d] (if (and (not term?) (proj/withdrawn-i? idx d)) (conj acc (str "depends_on points at abandoned " d)) acc)) v-thread (proj/string-values-at idx te "depends_on"))]
  v-ab))
