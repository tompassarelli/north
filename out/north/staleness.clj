...
(defn time-stale [idx ^String today before?]
  (reduce (fn [acc te] (let [vu (proj/string-value-at idx te "valid_until")]
...
(defn edge-stale [idx]
  (reduce (fn [acc te] (if (proj/terminal-i? idx te) acc (reduce (fn [a p] (reduce (fn [b tgt] (if (some? (proj/string-value-at idx tgt "abandoned")) (conj b (->Review te p (str "→ " tgt " was abandoned — relationship may be stale"))) b)) a (proj/string-values-at idx te p))) acc edge-preds))) [] (proj/thread-subjects idx)))

(defn- later-edit-scope-tx [latest ^String l]
  (reduce (fn [m v] (if (and (= (:l v) l) (and (kc/vec-member? scope-preds (:p v)) (and (real-edit-frame? (:frame v)) (> (:tx v) m)))) (:tx v) m)) 0 latest))
...
(defn bars-missing [idx]
  (reduce (fn [acc te] (if (and (some? (proj/string-value-at idx te "committed")) (and (some? (proj/string-value-at idx te "driver")) (and (not (proj/terminal-i? idx te)) (empty? (proj/string-values-at idx te "done_when"))))) (conj acc (->Review te "done_when" "committed + driven with no done bar — tell it done_when \"<probe + expected result>\"")) acc)) [] (proj/thread-subjects idx)))

(defn bars-unevidenced [idx]
  (reduce (fn [acc te] (let [bars (proj/string-values-at idx te "done_when")]
  ...
  (if (empty? open) acc (conj acc (->Review te "outcome" (str (- (count bars) (count open)) "/" (count bars) " bar(s) evidenced — " (reduce (fn [s m] (if (str/blank? s) m (str s " · " m))) "" marks) "  (bar_evidence \"<bar> → <observed result>\")"))))) acc))) [] (proj/thread-subjects idx)))
...
(defn promotable [idx]
  (filterv (fn [te] (and (nil? (proj/string-value-at idx te "committed")) (and (not (proj/terminal-i? idx te)) (has-structure? idx te)))) (proj/thread-subjects idx)))
