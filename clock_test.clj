...
(def asserts2
  [;; two agents, concurrent OPEN sessions on different threads — neither blocks
   ...
   (asrt 177 "@client-rate:acme" "rate" "175")
   ...
   (asrt 211 "@client-rate:label" "rate" "msa-standard")
...
(def checks2
  [;; (a) two agents open concurrently — each sees ONLY its own; a fresh agent is
   ...
   ["client authority captures its numeric value"         (= (:rate acme-rate) "175")]
   ["arbitrary thread rate facts cannot influence authority" (= (:rate acme-rate) "175")]
   ...
   ["positive decimal rate validation accepts integers" (clk/positive-rate? "175")]
   ["positive decimal rate validation accepts fractions" (clk/positive-rate? "175.50")]
...
(def integration-checks
  (let [root (.getCanonicalPath (.getParentFile (io/file *file*)))
        ...
        fixture [(op-line 1 "@thread-a" "title" "A")
                 ...
                 (op-line 14 "@client-rate:badlabel" "rate" "msa-standard")
    ...
    (try
      (if (not (await-port port))
        ...
        (let [missing (clock-run root environment "in" "msa")
              bad-set (clock-run root environment "rate" "msa" "standard")
              zero-set (clock-run root environment "rate" "msa" "0")
              negative-set (clock-run root environment "rate" "msa" "-1")
              ...
              configured (clock-run root environment "rate" "msa" "175")
              read-rate (clock-run root environment "rate" "msa")
              clock-in (clock-run root environment "in" "msa")
              status (clock-run root environment "status")
              second-in (clock-run root environment "in" "msa")
          ...
          [["thread rates cannot authorize clock-in"
           ...
           ["invalid commands create no msa authority"
            (= (:status (clk/client-rate-authority before-config "msa")) "missing")]
           ...
           ["one-time rate command creates the owner authority"
            (str/includes? (:out configured) "billing rate configured at 175/h")]
           ["rate read returns the authoritative value"
            (str/includes? (:out read-rate) "billing rate: 175/h")]
           ["clock-in snapshots the configured rate"
            (str/includes? (:out clock-in) "rate 175/h")]
           ["status reports the one client session"
            (str/includes? (:out status) "clocked in for client msa")]
           ["a second human clock-in is refused"
            (str/includes? (:out second-in) "already clocked in for client msa")]
           ["exactly one human client_session is visible" (= 1 (count client-sessions))]
           ["client_session captures owner, actor, rate, and start"
            (and (= (k/one-i open-idx session "owner") "msa")
                 (= (k/one-i open-idx session "clocked_by") "user")
                 (= (k/one-i open-idx session "rate") "175")
           ...
           ["clock-out closes the client session"
            (and (str/includes? (:out clock-out) "clocked out of client msa")
