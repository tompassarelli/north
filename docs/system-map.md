...
| `north-state-hosts-store` | `north-runtime-state` → `live-store-process` | runtime data and selector | ownership, activation, coordination | The service loses its local data or selected-runtime state. | `north:cli/store-runtime-generation.clj` |
