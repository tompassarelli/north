...
> - **One project/CLI/engine: `lodestar`.** `los thread`/`los validate` are
>   **retired** — use `lodestar` (ready/blocked/next/agenda/plate/show/leverage/
>   validate/capture/tell/untell/import/export/audit/doctor/up). Time tracking is
>   **`lodestar clock`** (claim-native sessions; `los time` and the JSON clock-in
...
This directory is the canonical thread layer for lodestar. Read this before
...
**The source of truth is the Lodestar claim graph** (`lodestar-data/claims.log`),
not the files. The `threads/*` files are a *faithful projection* of the claims,
regenerated by `lodestar export`; the import↔export round-trip is lossless
(claim-identical), so you can still edit a file and `lodestar import` folds the
...
the pre-graph markdown era is at `pre-lodestar-flip`.)
...
2026-06-15 — captured via `lodestar capture`.
```
...
ones in live use (run `lodestar show <id>` on a few real threads to see them):
...
ceremony beyond `title`. `lodestar export` writes a stable order; hand-edits
...
- The on-disk filename pairs the id with a snake_case slug. (Current corpus
  ...
  *inside* is dashed. The id inside the file is canonical; let `lodestar export`
  own filenames rather than hand-renaming.)
- The id never changes. The slug portion may be renamed (rare); the id stays put.
- Hand-minting an id: `date +%Y-%m-%d-%H%M%S`. But prefer `lodestar capture`,
  which mints it for you.
- `lodestar show <input>` resolves by id, then slug, then substring — so
  `lodestar show gjoa` finds the gjoa thread.
...
The derived conditions you'll actually use (all `lodestar` projections):
...
Don't invent a handle inline; the node must exist. `lodestar validate` rejects
...
obvious topic into a real thread), use `lodestar merge <from> <to>` — the
...
One binary. Run it via the lodestar wrapper: **`~/code/lodestar/bin/lodestar`**.
The wrapper aims the Fram engine (`~/code/fram`) at lodestar's private data
(`FRAM_THREADS`/`FRAM_LOG` under `~/.local/state/lodestar/`, projected from the
canonical `lodestar-data/claims.log`) and sets capture provenance defaults.

`los` is **gone entirely** — `los thread`/`los validate` retired (use `lodestar`),
and time tracking is now **`lodestar clock`** (claim-native; see Clock management).
One CLI.

Run `~/code/lodestar/bin/lodestar` with no args for the authoritative usage line;
don't trust an enumeration here over the binary. The surface:

**Reads (instant off the warm daemon, ~1ms):**

```sh
lodestar ready       # committed, not blocked, not active, not done — do-able now
lodestar blocked     # waiting on a depends_on target
lodestar next        # the recommended next pull
lodestar agenda      # calendar projection: buckets by do_on (overdue/today/next N)
lodestar plate       # everything on your plate, bucketed by derived condition
lodestar leverage    # high-leverage threads (most unblocks downstream)
lodestar show <id>   # one thread's claims + body; resolves id/slug/substring
lodestar validate    # integrity check (see below)
lodestar audit       # corpus-health report
lodestar needs-review # belief-revision queue: judgments whose inputs moved
```
...
```sh
lodestar doctor      # health of the coordinator + log (session-start handshake)
lodestar up          # start/revive the coordinator on the canonical log
lodestar watch       # event stream (change triggers; promotion prompts)
```

### Writing safely under concurrent agents

lodestar is claim-backed and **other agents may be editing concurrently**. The
rules (also in the global CLAUDE.md):

1. **Session-start handshake:** run `lodestar doctor`. If DOWN/DEGRADED, run
   `lodestar up` to start the coordinator on the canonical log.
2. **New threads:** `lodestar capture "<title>"` (claim-first) — or create the
   file and run `lodestar import`. Distinct files don't collide, so file-edit +
   `import` is safe for whole new threads.
3. **Field changes on existing threads:** go through the coordinator with
   `lodestar tell` / `untell` (serialized, rule-checked, retries on conflict).
   **Do NOT use `lodestar set`** — it appends the log directly and races. (`set`
   exists; it's for single-writer/offline situations only.)
4. **Do NOT run `lodestar export` during concurrent work** — it regenerates
...
Reads are instant off the warm daemon (`lodestar serve`); writes serialize
...
`lodestar validate` checks the graph, not a YAML schema:
...
2. **Run the handshake** (`lodestar doctor`, `lodestar up` if needed) before
   coordinating threads.
3. **Default `source ai`** for anything you originate, not `tom`. Add yourself
   to `proposed_by` (`@claude`, `@claude-code`). The `bin/lodestar` wrapper
   defaults capture provenance to Tom; when *you* originate, override via env:
   `LODESTAR_SOURCE=ai LODESTAR_AUTHOR=claude-code LODESTAR_DRIVER=claude-code
   LODESTAR_PROPOSED_BY=claude-code lodestar capture "..."` (lead stays Tom).
...
7. **Prefer the coordinator for field changes** — `lodestar tell`/`untell`, not
   `lodestar set`. Run `lodestar validate` after a batch.
...
- **Dangling refs.** Every `@`-ref must point at a node that exists; `validate`
  rejects otherwise.
- **`lodestar set` under concurrency.** Use `tell`/`untell` through the
  coordinator.
- **`lodestar export` during concurrent work.** It clobbers un-imported edits.
...
2. **Activate on engagement, not on mention.** A committed-but-dormant thread Tom
   ...
   not silently. Offer it; `lodestar tell <id> driver @claude-code` (or his
   handle) is the act.
...
Time tracking is **`lodestar clock`** — claim-native, *in the graph*. There is
...
```sh
lodestar clock start <thread-id>   # open a session (refuses if one's running)
lodestar clock stop                # close it, report the duration
lodestar clock status              # the running session + elapsed
lodestar clock report              # est vs actual per thread + calibration %
lodestar clock today | week        # logged time per thread over a date window
```
...
```sh
lodestar clock map <owner> <project-id>   # owner -> Clockify project (config)
lodestar clock sync                        # push closed, billable sessions
lodestar clock projects | workspaces       # list Clockify projects/workspaces
```
...
it's idempotent. Mapping lives in `~/code/lodestar/time/projects.json`; the API
...
- `~/code/lodestar/streams/raw/` — lossless transmission events (conversations,
  dictated thoughts, captured sessions).
- `~/code/lodestar/streams/distillations/` — tiered AI compressions of raw
  streams. See `streams/CLAUDE.md`.
- `~/code/lodestar/lodestar-data/claims.log` — the canonical claim graph (source
  of truth; `threads/` is its projection).
- `~/code/lodestar` — the generic engine (public source of truth); `bin/lodestar`
  is lodestar's consumer wrapper.
