...
**The source of truth is the Tern fact graph** (`tern-data/claims.log` — the
filename keeps its legacy `claims.log` spelling; the data unit it stores is a
fact), not the files. The `threads/*` files are a *faithful projection* of the
facts, regenerated by `tern export`; the import↔export round-trip is lossless
(fact-identical), so you can still edit a file and `tern import` folds the
edit back into the graph. If a tool and the files disagree, reconcile through the
fact log — `import` to absorb file edits, `export` to regenerate files — and the
fact graph wins. (Pre-cutover state is recoverable at git tag `pre-claim-native`;
...
- `~/code/tern/tern-data/claims.log` — the canonical fact graph (source
  of truth; `threads/` is its projection). (The filename keeps its legacy
  `claims.log` spelling; the data unit it stores is a fact.)
