...
Task is delegated realizing work: a proposed action/outcome obligation carried by a TaskSpec and assignment occurrence. Objective is the bounded desired condition. Plan→Project is adoption of a value, not identity
mutation. Thread is the continuity ledger for messages, intentions, facts, dependencies, and outcomes (conversation is optional), and may be `about` any referent; it does
...
`execution_attempt_run`; canonical identity is `Objective → Run (semantic Attempt) → DeliveryAttempt (opaque referent for one concrete reservation)`. `runId` is Attempt ID; the manifest hash is reservation content, never identity. Attempt is created at admission with exactly one Assignment and may have zero or more DeliveryAttempts. Provider-reservation replacement creates a new DeliveryAttempt under the same Attempt; handoff/reassignment creates a new Assignment and successor Attempt under the same Objective; material scope/acceptance change creates a successor Objective and its Attempt. Failed reservation is reconciled by idempotency key and does not create a second semantic Attempt. Phase 0 proves these cardinalities and the zero-reservation state.
...
The first writer is typed `admit-objective-run`: one Store transaction admits Objective, Task obligation, Assignment, and semantic Attempt in coordination only. Telemetry propagation and provider reservation occur afterward, idempotently keyed to the committed snapshot; the host executes reservation as intent→authorization→attempt→receipt. Coordination and telemetry snapshots are explicit and mismatches are unknown. Implement only exact Objective/Assignment/Attempt joins, separate forecast and budget, turn and
...
from Store facts alone. Reuse Wire event IDs/sequences for turn/test occurrences; add no second ledger. Non-Objective Wire producers remain observations until admitted. Check in positive and negative proving fixtures; ambiguous joins yield unknown/exclusion.

## Migration and recovery

Forward-only current-main migration. New writes require explicit base kind and canonical targets.
Ambiguous history is `legacy_unclassified` and inert; classify only where producer provenance proves type. Active/recovery-relevant ambiguous rows require explicit operator re-admission as new canonical referents. Migrate consumers in
...
Freeze existing run-scoped learning before Phase 0: reject or force-control new non-Objective exposures, prove no post-cutoff exploratory assignments, then resume report-only evaluation only after Objective admission and coverage gates pass. Randomize at Objective level, one
