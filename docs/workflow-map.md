...
|------|---------------------|
| **fact** | a *status* North's domain confers, never the name of the stored thing. North's coordination rows are performative — writing "this lease is held" constitutes the state it describes — so fact-hood is honest here. The stored unit is Store's recursive Triple (three neutral slots, any of which may hold another Triple) plus the occurrence that asserted or retracted it; subject/predicate/object is a domain reading, never kernel law (`store:docs/ontology.md`) |
