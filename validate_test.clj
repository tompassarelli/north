;; validate_test.clj — lodestar's WORK-semantics integrity rules, lifted out of
;; the fram kernel into lodestar.validate: a depends_on edge to a withdrawn
;; (abandoned) thread, and person-ref integrity (lead/driver/proposed_by must
;; point at a node carrying a `display_name`). Plus: lodestar.validate composes
...
(require '[fram.kernel :as k] '[lodestar.validate :as val])
...
;; composition: full violations-i = engine-generic ++ lodestar-work.
(def mixed-claims
  [(k/->Claim "@w5" "title" "W5")
...
(let [fails (remove second checks)]
  ...
  (if (empty? fails)
    (println "\nlodestar.validate:" (count checks) "/" (count checks) "PASS")
    (do (println "\nlodestar.validate:" (count fails) "FAILED") (System/exit 1))))
