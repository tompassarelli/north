;; lifecycle_test.clj — the live-corpus lifecycle base expressed as Datalog over
;; the reified store must match the canonical flat projection. Driver liveness and
...
;; The Datalog twin intentionally has no clock or lease axis. A far-future today
;; removes dormancy and an injected always-false live? treats durable drivers as
;; parked assignments. The deterministic schedule/liveness precedence is covered
;; directly in projections_test.clj.
(def flat-idx (k/build-index flat-facts))
(def far-future "9999-12-31")
(defn before? [a b] (neg? (compare a b)))
(defn no-live? [_idx _te] false)
(def flat-ready (set (proj/ready flat-idx far-future before? no-live?)))
...
(def terminal-ps
  (vec (remove nil? (map (fn [p] (c/value-id ctx p))
...
;; Mirrors north.projections/anchor-i? exactly, so flat work == reified work both pre-
;; AND post-migration (after source=migrated is dropped).
(def work-axis-preds
  (->> ["outcome" "abandoned" "superseded_by" "driver" "depends_on" "part_of" "do_on"
...
(def strata
  [;; stratum 0 — positive base over the EDB
   (into
    (vec
     (concat
      ...
      (map (fn [tp]
             (d/rule "terminal" [(d/v :x)] [(d/lit "triple" [(d/v :x) tp (d/v :term)])]))
           terminal-ps)))
   ...
   [(d/rule "blocked" [(d/v :x)] [(d/lit "work" [(d/v :x)]) (d/nlit "terminal" [(d/v :x)]) (d/lit "open-dep" [(d/v :x)])])
    (d/rule "ready"   [(d/v :x)] [(d/lit "work" [(d/v :x)]) (d/lit "committed-r" [(d/v :x)])
                                    (d/nlit "terminal" [(d/v :x)]) (d/nlit "open-dep" [(d/v :x)])])]])
...
(let [fails (remove second checks)]
  ...
  (if (empty? fails)
    (println "\nlifecycle base: Datalog == canonical projection PASS")
    (do (println "\nlifecycle base: FAIL") (System/exit 1))))
