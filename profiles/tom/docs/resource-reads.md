# Resource reads — vetted takeaways from ~/code/resources forks

Vetted pointers from resource forks. Licenses verified — all MIT; one
carve-out in License notes.

## Skill authoring — read BEFORE writing a new skill

- `~/code/resources/superpowers/skills/writing-skills/SKILL.md` — the best
...
- `~/code/resources/superpowers/skills/writing-skills/testing-skills-with-subagents.md`
...
- `~/code/resources/superpowers/skills/writing-skills/persuasion-principles.md`
  — persuasion research applied to making discipline-enforcing skills stick.

## Debugging techniques — reusable patterns

- `~/code/resources/superpowers/skills/systematic-debugging/find-polluter.sh`
  — bisects a test suite to find which test pollutes global state. Grab verbatim.
- `~/code/resources/superpowers/skills/systematic-debugging/condition-based-waiting.md`
  (+ `-example.ts`) — replace arbitrary timeouts with condition polling; the
  cure for flaky async tests.
- `~/code/resources/superpowers/skills/systematic-debugging/root-cause-tracing.md`
  — backward-trace through the call stack with a worked 5-level example.
- `~/code/resources/superpowers/skills/systematic-debugging/defense-in-depth.md`
...
- `~/code/resources/Cortex/mcp_server/core/response_budget.py` — EMPIRICALLY
...
- `~/code/resources/Cortex/mcp_server/core/gist_extraction.py` — deterministic
...
- `~/code/resources/Cortex/mcp_server/core/context_assembly/condensers.py` —
  domain-aware condensing: code blocks verbatim, prose compressed,
  first + question + last sentences kept.
- `~/code/resources/ponytail/hooks/ponytail-runtime.js` `writeHookOutput()` —
...
- `~/code/resources/ponytail/benchmarks/` — real headless Claude Code sessions,
...
- `~/code/resources/honey-for-devs/bench/` — multi-model judge panel (4-model
...
- `~/code/resources/honey-for-devs/skills/honey-memory/SKILL.md` — per-project
...
All four repos MIT. Exception: `~/code/resources/honey-for-devs/hooks/eco.js` +
