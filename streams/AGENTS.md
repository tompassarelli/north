...
- `streams/raw/` — **lossless transmission events**: full provider-native
  session transcripts, dictated thoughts, captured conversations.
...
- **Raw capture = byte copy, zero tokens.** Provider harnesses write their
  native transcript files and `north-stream-sync-all` discovers ambient
  provider homes, provider accounts, profiles, and managed launches. All land
  in this same raw directory. Each explicit provider
...
- **Distillation = economy-tier agent**, never the
