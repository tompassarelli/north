...
function WorkItem(id, title, condition, driver, text, kanban_text) {
  return Object.freeze({_tag: "WorkItem", id, title, condition, driver, text, kanban_text});
}
...
function workitem_text(r) { return r.text; }

function workitem_kanban_text(r) { return r.kanban_text; }

function BridgeModel(agents, transcript, graph, kanban, layout, active_view_id, selected_agent, selected_thread, filter_text, notice) {
  return Object.freeze({_tag: "BridgeModel", agents, transcript, graph, kanban, layout, active_view_id, selected_agent, selected_thread, filter_text, notice});
}
...
function bridgemodel_graph(r) { return r.graph; }
...
export function make_work(id, title, condition, driver, text, kanban_text) {
  return WorkItem(id, title, condition, driver, text, kanban_text);
}

export function make_model(view_id) {
  return BridgeModel([], [], [], [], "vertical", ((view_id === "kanban") ? "kanban" : "graph"), "", "", "", ("".concat("view ", view_id)));
}

export function replace_projection(model, agents, graph, kanban) {
  return BridgeModel(agents, bridgemodel_transcript(model), graph, kanban, bridgemodel_layout(model), bridgemodel_active_view_id(model), bridgemodel_selected_agent(model), bridgemodel_selected_thread(model), bridgemodel_filter_text(model), bridgemodel_notice(model));
}
...
export function focus_view(model, view_id) {
  return BridgeModel(bridgemodel_agents(model), bridgemodel_transcript(model), bridgemodel_graph(model), bridgemodel_kanban(model), bridgemodel_layout(model), ((view_id === "kanban") ? "kanban" : "graph"), bridgemodel_selected_agent(model), bridgemodel_selected_thread(model), bridgemodel_filter_text(model), bridgemodel_notice(model));
}

export function set_layout(model, layout) {
  return BridgeModel(bridgemodel_agents(model), bridgemodel_transcript(model), bridgemodel_graph(model), bridgemodel_kanban(model), ((layout === "horizontal") ? "horizontal" : "vertical"), bridgemodel_active_view_id(model), bridgemodel_selected_agent(model), bridgemodel_selected_thread(model), bridgemodel_filter_text(model), bridgemodel_notice(model));
}

export function select_agent(model, agent_id) {
  return BridgeModel(bridgemodel_agents(model), bridgemodel_transcript(model), bridgemodel_graph(model), bridgemodel_kanban(model), bridgemodel_layout(model), bridgemodel_active_view_id(model), agent_id, bridgemodel_selected_thread(model), bridgemodel_filter_text(model), ((agent_id === "") ? "showing all work" : ("".concat("work assigned to ", agent_id))));
}

export function select_thread(model, thread_id) {
  return BridgeModel(bridgemodel_agents(model), bridgemodel_transcript(model), bridgemodel_graph(model), bridgemodel_kanban(model), bridgemodel_layout(model), bridgemodel_active_view_id(model), bridgemodel_selected_agent(model), thread_id, bridgemodel_filter_text(model), ((thread_id === "") ? "no thread selected" : ("".concat("selected @", thread_id))));
}
...
export function snapshot(model) {
  const graph = visible_work(model, bridgemodel_graph(model));
  const kanban = visible_work(model, bridgemodel_kanban(model));
  return {agents: bridgemodel_agents(model), transcript: bridgemodel_transcript(model), graph: graph, kanban: kanban, views: [{id: "graph", title: "Threads DAG", items: graph}, {id: "kanban", title: "Kanban", items: kanban}], layout: bridgemodel_layout(model), activeViewId: bridgemodel_active_view_id(model), selectedAgent: bridgemodel_selected_agent(model), selectedThread: bridgemodel_selected_thread(model), notice: bridgemodel_notice(model)};
}
