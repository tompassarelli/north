...
function active_route(state) {
  ...
  return Object.assign(Object(), $$bh$js_obj("provider", routing.provider, "providerTarget", routing.target, "liveInput", (providers_module.providerLiveInput)(routing.provider)), defined_field("model", ((_logical) => (_logical !== false && _logical != null ? _logical : opts.model))(routing.resolvedModel)), defined_field("effort", ((_logical) => (_logical !== false && _logical != null ? _logical : opts.routingMetadata.reasoning))(routing.resolvedEffort)));
}

function defined_field(key, value) {
  return (((value == null) || (value === undefined)) ? Object() : $$bh$js_obj(key, value));
}
...
async function run_spawn_bang(opts, judgment_grade, struggle_policy, envelope_admission, injected, termination, worktree_lease, retry_context, retry_target, parent_run_id, learning_assignment, shadow_config, lifecycle_journal) {
  ...
  const identity_base = Object.assign(Object(), $$bh$js_obj("kind", "lane", "role", identity_role, "compositionKind", composition.kind, "compositionId", composition.id, "repo", (identity_module.userAnchoredPath)(process.cwd()), "goal", (identity_module.goalFromPrompt)(opts.prompt)), defined_field("compositionOverrides", (template_p ? composition.overrides : null)), defined_field("compositionOverrideReason", (template_p ? composition.overrideReason : null)), defined_field("compositionNearestTemplate", (bespoke_p ? composition.nearestTemplate : null)), defined_field("compositionBespokeReason", (bespoke_p ? composition.bespokeReason : null)), defined_field("compositionPromotionCandidate", (bespoke_p ? composition.promotionCandidate : null)), defined_field("compositionContractFingerprint", (bespoke_p ? (identity_module.bespokeContractFingerprint)(composition.contract) : null)), defined_field("compositionContractFingerprintVersion", (bespoke_p ? bespoke_contract_module.BESPOKE_FINGERPRINT_VERSION : null)), defined_field("compositionContractFingerprintDomain", (bespoke_p ? bespoke_contract_module.BESPOKE_FINGERPRINT_DOMAIN : null)), defined_field("coordinator", opts.coordinator), defined_field("worktree", (((_truthy) => _truthy !== false && _truthy != null)(worktree_lease) ? worktree_lease.path : null)), defined_field("branch", (((_truthy) => _truthy !== false && _truthy != null)(worktree_lease) ? worktree_lease.branch : null)), defined_field("retryOfAgent", (((_truthy) => _truthy !== false && _truthy != null)(retry_context) ? retry_context.retryOfAgent : null)), defined_field("shadowReviewerNoteCapabilitySha256", (((_truthy) => _truthy !== false && _truthy != null)(shadow_note_capability) ? shadow_note_capability.sha256 : null)));
  ...
  const live_input_route = foreign_new_bang(live_input_route_module, "ManagedLiveInputRoute", $$bh$array(agent_id, identity_base, Object.assign(Object(), $$bh$js_obj("provider", routing.provider, "providerTarget", routing.target, "liveInput", initial_live_input), defined_field("model", resolved_model), defined_field("effort", resolved_effort)), (message) => channel.push(message), ((_logical) => (_logical !== false && _logical != null ? _logical : coordination_module.subscribeFeed))(injected.feedSubscriber)));
  await (identity_module.writeAgentFacts)(agent_id, Object.assign(Object(), identity_base, Object.assign(Object(), $$bh$js_obj("provider", routing.provider, "providerTarget", routing.target, "liveInput", initial_live_input), defined_field("model", resolved_model)), live_input_route.initialProjection(), defined_field("effort", resolved_effort)));
  if (((_truthy) => _truthy !== false && _truthy != null)(lifecycle_journal)) {
