...
export const RETIRED_ORCHESTRATION_ROLE_IDS = new Set(["researcher", "research-scientist", "cs-researcher"]);
...
export function requireOrchestrationRoleId(value: unknown, label = "role"): string {
  if (typeof value === "string" && RETIRED_ORCHESTRATION_ROLE_IDS.has(value)) {
    throw new Error(`role ${value} is retired; use scout, analyst, or scientist`);
  }
