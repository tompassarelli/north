...
test("resolved generation presents recursive module edges alphabetically", () => {
  ...
  expect(tree).toContain("module agent-machinery: on · permission on · owner agent-machinery:catalog.json");
  expect(tree).toContain("module coordination: on · permission on · owner north:agent-catalog/modules/coordination.json");
  expect(tree.indexOf("skill agent-run-lifecycle-distilled")).toBeLessThan(tree.indexOf("skill threads-distilled"));
  expect(tree.match(/module coordination:/g)).toHaveLength(2);
  expect(tree).toContain("hook worktree-guard: on · permission on");
  expect(tree).toContain("skill agent-run-lifecycle-distilled: on · permission on · owner north:coordination/agent-run-lifecycle-distilled/SKILL.md · also in operations");
  expect(tree).toContain("activation: workspace → coordination → agent-run-lifecycle-distilled");
  expect(tree).toContain("activation: workspace → planning → coordination → agent-run-lifecycle-distilled");
});
