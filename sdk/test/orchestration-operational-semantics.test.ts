...
test("Orchestration capabilities compile to exact provider authority before work starts", () => {
  ...
  expect(loggedSurface).toBe(
    ...
    + "north enabled_tools=capture,tell,evidence_record,show,search,artifact_read,ready,next; "
...
test("Codex receives per-run native-agent disablement and a worker North allowlist", () => {
  ...
  expect(worker).toEqual([
    ...
    "mcp_servers.north.enabled_tools=[\"capture\",\"tell\",\"evidence_record\",\"show\",\"search\",\"artifact_read\",\"ready\",\"next\"]",
  ]);
