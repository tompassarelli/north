...
;; reified-store loader + the hand-listed predicate schema — a cost lodestar does NOT pay today.
;;   FRAM_LOG=~/.local/state/lodestar/claims.log bb -cp out leverage_probe.clj
...
;; --- reified-store loader (the scaffolding (B) requires; lodestar pays NONE of this today) ---
...
(println "      schema + def-predicate! loop + ent-for! memo + ref? + load loop) that lodestar pays NOWHERE today.")
