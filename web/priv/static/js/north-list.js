...
(function () {
  ...
  const focusGraph = (id) => { if (window.north && window.north.focusGraph) window.north.focusGraph(id); };
  const ACTIONS = [
  ...
  window.north.mountList = function ({ el: root }) {
    if (!root) return;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (root) window.north.mountList({ el: root });
  });
})();
