...
(function () {
  ...
  // Matches north-list.js exactly so the dot/accent reads identically.
  ...
  // monospace, leads the title — matches north-list.js's handleChip exactly.
  ...
  // Same routine as north-list.js's dragAfter.
  ...
  async function render(root) {
    ...
    // Delegated drag lifecycle (mirrors north-list.js: dragstart/dragover/dragend).
  ...
  window.north.mountBoard = function ({ el: root }) {
    if (!root) return;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (root) window.north.mountBoard({ el: root });
  });
})();
