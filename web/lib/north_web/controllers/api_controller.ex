defmodule NorthWeb.ApiController do
  ...
  def dag(conn, %{"focus" => focus}) when is_binary(focus) and focus != "",
    do: json(conn, North.Threads.focused(focus))

  def dag(conn, _params), do: json(conn, North.Threads.graph())
  ...
  def presence(conn, _params) do
    roster = North.Presence.roster()
    json(conn, %{agents: roster, swarm: North.Presence.swarm_tokens(roster)})
  end
  ...
  def assert(conn, %{"graph" => g, "te" => te, "p" => p, "r" => r}) do
    write_resp(conn, North.Fram.assert!(North.Fram.port_for(g), te, p, r))
  end

  def retract(conn, %{"graph" => g, "te" => te, "p" => p, "r" => r}) do
    write_resp(conn, North.Fram.retract!(North.Fram.port_for(g), te, p, r))
  end

  # Higher-level fact-native verb (north `tell`): one fact about an entity.
  ...
  def tell(conn, %{"graph" => g, "id" => id, "pred" => pred, "obj" => obj}) do
    te = North.Threads.resolve(at(id))
    r = if String.starts_with?(obj, "@"), do: North.Threads.resolve(obj), else: obj
    write_resp(conn, North.Fram.assert!(North.Fram.port_for(g), te, pred, r))
  end

  # Capture a new thread (north `capture`): mint a UUIDv7 id + assert its title
  ...
  def capture(conn, %{"title" => title} = params) when is_binary(title) and title != "" do
    port = North.Fram.port_for(Map.get(params, "graph", "board"))
    id = "@" <> North.Id.uuid7()
    now = DateTime.utc_now() |> DateTime.to_iso8601()

    with {:ok, _} <- North.Fram.assert!(port, id, "title", title),
         {:ok, v} <- North.Fram.assert!(port, id, "created_at", now),
  ...
  def steer(conn, %{"handle" => h, "text" => text}) when is_binary(h) and is_binary(text) and text != "" do
    handle = h |> at() |> North.Threads.resolve() |> String.replace_prefix("@", "")
    write_resp(conn, North.Fram.assert!(North.Fram.agents_port(), "@session:" <> handle, "steer", text))
  end
  ...
  defp assert_optional(port, id, pred, v) when is_binary(v), do: North.Fram.assert!(port, id, pred, v)
  ...
  defp assert_optional_ref(port, id, pred, v) when is_binary(v),
    do: North.Fram.assert!(port, id, pred, North.Threads.resolve(at(v)))

  # wake's sibling /live WebSocket — upgrade + hand off to LiveFeed (PubSub-fed).
  def live(conn, _params) do
    conn
    |> WebSockAdapter.upgrade(NorthWeb.LiveFeed, [], timeout: 120_000)
    |> halt()
  end
  ...
  <body>
    ...
    <script src="/js/north-ui.js"></script>
  </body>
  </html>
  """
  ...
  def board_view(conn, _params),
    do: conn |> put_resp_content_type("text/html") |> send_resp(200, shell("board", "board", "/js/north-board.js"))

  def agents_view(conn, _params),
    do: conn |> put_resp_content_type("text/html") |> send_resp(200, shell("agents", "agents", "/js/north-agents.js"))
  ...
  def arena_view(conn, _params),
    do: conn |> put_resp_content_type("text/html") |> send_resp(200, shell("arena", "arena", "/js/north-arena.js"))

  # The full 2-panel client (workbench + agents). Loads every renderer; the
  # orchestrator (north-app.js, last) lays out the frames + view toggle.
  @app_shell """
  <!doctype html>
  <html lang="en">
  <head>
  ...
  <body>
    ...
    <script src="/js/north-app.js"></script>
  </body>
  </html>
  """
