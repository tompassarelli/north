defmodule North.GraphCache do
  ...
  @impl true
  def init(_) do
    Phoenix.PubSub.subscribe(North.PubSub, "wakefeed")
    {:ok, %{fold: nil, port: Fram.board_port()}}
  end

  @impl true
  def handle_call({:fold, port}, _from, %{fold: nil} = st),
    do: (f = North.Threads.fold_triples(port); {:reply, f, %{st | fold: f, port: port}})
  ...
  def handle_info({:commit, "board"}, st) do
    me = self()
    port = st.port
    Task.start(fn -> send(me, {:set, North.Threads.fold_triples(port)}) end)
    {:noreply, st}
  end
