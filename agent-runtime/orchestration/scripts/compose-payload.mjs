#!/usr/bin/env node
// Assembles a stock-template lane's provider-specific runtime payload for
// custom dispatch surfaces:
...
const MACHINERY_ROOT = resolve(process.env.AGENT_MACHINERY_HOME ??
  resolve(process.env.HOME ?? "", "code/agent-machinery/main"));
const machinery = await import(pathToFileURL(resolve(MACHINERY_ROOT, "index.mjs")));
const { canonicalRoleId } = await import(pathToFileURL(resolve(MACHINERY_ROOT, "scripts/role-id.mjs")));
const { block, firstFence } = await import(pathToFileURL(resolve(MACHINERY_ROOT, "scripts/blocks.mjs")));
const readMachinery = (path) => readFileSync(resolve(MACHINERY_ROOT, path), "utf8");
const readRuntime = (path) => readFileSync(resolve(ROOT, path), "utf8");
...
function main() {
  ...
  const staffing = machinery.loadStaffingCatalog();
  ...
  const parts = [
    ...
    "",
    "## Role",
    block(readMachinery("docs/roles.md"), role),
    "",
    `## Task grade: ${preset.taskGrade}`,
    block(readMachinery("docs/task-grades.md"), preset.taskGrade),
    "",
    `## Topology: ${preset.topology}`,
    block(readMachinery("docs/topologies.md"), preset.topology),
    "",
    `## Posture: ${preset.posture}`,
    block(readMachinery("docs/postures.md"), preset.posture),
    "",
    "## Output norms",
    block(readMachinery("docs/comms.md"), "universal"),
  ];
  ...
  if (familyPath && !flags.noFamily) {
    ...
    parts.push("", `## Provider family protocol (${provider})`, firstFence(readRuntime(familyPath)));
  }

  if (steering !== "light") {
    const delta = modelDeltaFor(catalog, resolved.model);
    if (delta.kind === "calibrated")
      parts.push("", "## Delta protocol — tuned to this model's documented tendencies", firstFence(readRuntime(delta.path)));
    else
